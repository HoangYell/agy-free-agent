...
if command -v agy &>/dev/null || [[ -x "${BIN_DIR}/agy" ]]; then
  ...
  if [[ -z "${INSTALL_AGY}" && "${INTERACTIVE}" == "true" ]]; then
    ...
    echo -ne "  ${CYAN}➜${RESET} Install official Google Antigravity CLI now? [Y/n]: "
    read_prompt "" INSTALL_AGY
    INSTALL_AGY="${INSTALL_AGY:-y}"
  fi

  if [[ "${INSTALL_AGY}" =~ ^[Yy]|true$ ]]; then
    echo -e "  ${CYAN}ℹ Installing official Antigravity CLI via https://antigravity.google/cli/install.sh...${RESET}"
    curl -fsSL https://antigravity.google/cli/install.sh | bash || true
  fi

  if ! command -v agy &>/dev/null && [[ ! -x "${BIN_DIR}/agy" ]]; then
    echo -e "  ${SLATE}Run: curl -fsSL https://antigravity.google/cli/install.sh | bash${RESET}"
  fi
fi
